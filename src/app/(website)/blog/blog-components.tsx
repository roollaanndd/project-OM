...
export function BlogFeatured({ post }: { post: BlogPost }) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group relative block overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-soft-pink transition-all hover:shadow-glow-pink card-tilt"
    >
      <div className="grid lg:grid-cols-2">
        ...
        <div className="flex flex-col justify-center p-6 lg:p-8">
          <div className="flex items-center gap-3 text-xs text-gray-600">
            <span className="rounded-full bg-gray-100 px-2.5 py-0.5 font-bold">{post.category}</span>
          ...
          <h2 className="mt-3 font-display text-2xl font-extrabold leading-tight text-gray-900 lg:text-3xl">
            {post.title}
          </h2>
...
export function BlogCard({ post, index }: { post: BlogPost; index: number }) {
  return (
    ...
    >
      <Link
        href={`/blog/${post.slug}`}
        className="group flex h-full flex-col overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-soft-pink card-premium"
      >
        <div className="relative aspect-video overflow-hidden">
          <Image
            ...
            loading="lazy"
          />
          <span className="absolute left-3 top-3 rounded-full bg-white/90 px-2.5 py-0.5 text-[10px] font-bold text-gray-700 backdrop-blur">
            {post.category}
          </span>
        </div>
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-3 flex items-center gap-1 text-xs font-bold text-gray-700">
            Baca
...
export function BlogHero() {
  return (
    <m.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
      <span className="glass inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold text-gray-800 shadow-sm">
        <Sparkles className="h-3.5 w-3.5" />
        Blog OMDC
      </span>
      <h1 className="mt-5 font-display text-4xl font-extrabold leading-tight text-gray-900 sm:text-5xl lg:text-6xl">
        Tips & Panduan
        <br />
