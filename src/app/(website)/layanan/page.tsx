    ...
    <main className="min-h-screen bg-gray-50">
      <section className="mx-auto max-w-6xl px-5 py-14 lg:px-8 lg:py-20">
        <nav aria-label="Breadcrumb" className="mb-6 text-xs text-gray-900/55">
          ...
          <span className="font-bold text-gray-700">Layanan</span>
        </nav>
        ...
        <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {SERVICES.map((s) => (
            <Link
              key={s.slug}
              href={`/layanan/${s.slug}`}
              className="group relative overflow-hidden rounded-3xl border border-gray-100 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-soft-pink"
            >
              ...
              <h2 className="mt-4 font-display text-lg font-bold text-gray-900">{s.name}</h2>
              <p className="mt-1 text-sm italic text-gray-700">{s.tagline}</p>
